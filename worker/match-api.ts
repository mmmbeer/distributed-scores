...
function validSetWinner(match: Match, winner: TeamKey) { const winnerPoints = winner === "a" ? match.teamA.points : match.teamB.points; const loserPoints = winner === "a" ? match.teamB.points : match.teamA.points; if (match.sport === "badminton") return (winnerPoints >= 21 && winnerPoints - loserPoints >= 2) || (winnerPoints === 30 && winnerPoints > loserPoints); return winnerPoints >= match.currentTarget && winnerPoints - loserPoints >= 2; }
...
async function updateMatch(request: Request, env: MatchEnv, code: string) {
  ...
  if (action === "point") {
    ...
    if (SPORTS[current.sport].unit !== "set" || current.sport === "tennis") throw new ApiError(409, "This sport does not use manual game wins");
